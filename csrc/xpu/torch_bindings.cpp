...
TORCH_LIBRARY_EXPAND(TORCH_EXTENSION_NAME, xpu_ops) {
  ...
  xpu_ops.def(
      "fp8_block_scaled_gemm(Tensor A, Tensor B, Tensor A_scale, Tensor "
      "B_scale, int block_n, int block_k, ScalarType? out_dtype, Tensor? "
      "bias_) -> Tensor");
  xpu_ops.impl("fp8_block_scaled_gemm", torch::kXPU, &fp8_block_scaled_gemm);
